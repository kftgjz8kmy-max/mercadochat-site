...
export default function Home() {
  ...
  return <main>
    ...
    <section id="planes" className="pricing shell"><SectionTitle>Planes para cada tipo de operación</SectionTitle><p className="pricing-intro">Empieza gratis. Elige tu plan cuando estés listo.<br />Prueba merchat durante 14 días y conoce todo lo que puede hacer por tu operación antes de decidir cómo continuar.<br /><span>Precios promocionales de lanzamiento: 50% de descuento en todos los planes.</span></p><div className="pricing-grid">{landing.plans.map((plan) => <article className={`price-card ${plan.featured ? 'featured' : ''}`} key={plan.name}>{plan.featured && <span className="popular">RECOMENDADO</span>}<p className="discount-badge"><svg className="discount-icon" style={{ width: 14, height: 14, flex: "none" }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="m20 13-7 7a2 2 0 0 1-2.8 0l-6.2-6.2a2 2 0 0 1 0-2.8V4h7.8a2 2 0 0 1 1.4.6l6.2 6.2a2 2 0 0 1 0 2.2Z"/><circle cx="8" cy="8" r="1.2" fill="currentColor" stroke="none"/></svg>50% DE DESCUENTO</p><h3>{plan.name}</h3><p className="audience">{plan.audience}</p><p className="price"><del>S/{plan.originalPrice}</del><small>{plan.prefix}</small>S/{plan.price}<small> /mes</small></p><p className="setup">{plan.setup}</p><ul>{plan.items.map((item) => <li className={item.startsWith("Todo lo incluido") ? "feature-inherited" : undefined} key={item}><svg className="feature-check" viewBox="0 0 20 20" aria-hidden="true"><path d="m4.2 10.2 3.5 3.5 8-8" /></svg><span>{item}</span></li>)}</ul><Button outline={!plan.featured} href={plan.name === "Vendedor Pro" ? siteConfig.trialUrl : siteConfig.whatsappUrl}>{plan.cta}</Button>{plan.name === "Vendedor Pro" && <p className="plan-note">Sin tarjeta</p>}</article>)}</div></section>
